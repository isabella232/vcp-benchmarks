...
func main() {
	...
	for i := 0; i < (numurls * hitpct / 100); i++ {
		urls[i] = "GET http://vcpbench-elb-1056710711.eu-west-1.elb.amazonaws.com/" + strconv.FormatInt((size/factor), 10) + hiturl + strconv.FormatInt((size/factor), 10)
		size = size + delta
	}
	...
	for i := (numurls * hitpct / 100); i < numurls; i++ {
		urls[i] = "GET http://vcpbench-elb-1056710711.eu-west-1.elb.amazonaws.com/" + strconv.FormatInt((size/factor), 10) + missurl + strconv.FormatInt((size/factor), 10)
		size = size + delta
	}
